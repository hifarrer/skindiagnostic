...
function route(){var hash=location.hash.replace('#','')||'dashboard';if(!views[hash]){location.hash='#dashboard';return}currentView=hash;pageTitle.textContent=titles[hash];document.querySelectorAll('.nav-item[data-view]').forEach(function(el){el.classList.toggle('active',el.dataset.view===hash)});sidebar.classList.remove('open');destroyCharts();views[hash]()}
